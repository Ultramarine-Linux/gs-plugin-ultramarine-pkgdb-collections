...
#include "gs-plugin-ultramarine-pkgdb-collections.h"

/*
 * SECTION:
 * Queries the list of Ultramarine package collections.
 *
...
static void
gs_plugin_ultramarine_pkgdb_collections_init (GsPluginUltramarinePkgdbCollections *self)
{
	GsPlugin *plugin = GS_PLUGIN (self);

	/* check that we are running on Ultramarine */
	if (!gs_plugin_check_distro_id (plugin, "ultramarine")) {
		gs_plugin_set_enabled (plugin, FALSE);
		g_debug ("disabling itself as we're not Ultramarine");
		return;
	}
...
static void
gs_plugin_ultramarine_pkgdb_collections_dispose (GObject *object)
{
	GsPluginUltramarinePkgdbCollections *self = GS_PLUGIN_ULTRAMARINE_PKGDB_COLLECTIONS (object);

	g_clear_object (&self->cachefn_monitor);
	g_clear_object (&self->cached_origin);
	g_clear_object (&self->settings);

	G_OBJECT_CLASS (gs_plugin_ultramarine_pkgdb_collections_parent_class)->dispose (object);
}

static void
gs_plugin_ultramarine_pkgdb_collections_finalize (GObject *object)
{
	GsPluginUltramarinePkgdbCollections *self = GS_PLUGIN_ULTRAMARINE_PKGDB_COLLECTIONS (object);
...
_file_changed_cb (GFileMonitor *monitor,
		  ...
		  gpointer user_data)
{
	GsPluginUltramarinePkgdbCollections *self = GS_PLUGIN_ULTRAMARINE_PKGDB_COLLECTIONS (user_data);
...
static void
gs_plugin_ultramarine_pkgdb_collections_setup_async (GsPlugin            *plugin,
                                                ...
                                                gpointer             user_data)
{
	...
	g_task_set_source_tag (task, gs_plugin_ultramarine_pkgdb_collections_setup_async);

	/* get the file to cache */
	self->cachefn = gs_utils_get_cache_filename ("ultramarine-pkgdb-collections",
						     "ultramarine.json",
...
static gboolean
gs_plugin_ultramarine_pkgdb_collections_setup_finish (GsPlugin      *plugin,
                                                 GAsyncResult  *result,
                                                 GError       **error)
{
...
static void
_refresh_cache_async (GsPluginUltramarinePkgdbCollections *self,
...
static gboolean
_refresh_cache_finish (GsPluginUltramarinePkgdbCollections  *self,
                       GAsyncResult                    *result,
                       GError                         **error)
{
	return g_task_propagate_boolean (G_TASK (result), error);
...
static gboolean
gs_plugin_ultramarine_pkgdb_collections_refresh_metadata_finish (GsPlugin      *plugin,
                                                            GAsyncResult  *result,
                                                            GError       **error)
{
	return _refresh_cache_finish (GS_PLUGIN_ULTRAMARINE_PKGDB_COLLECTIONS (plugin),
				      result,
				      error);
}
...
static GsApp *
_create_upgrade_from_info (GsPluginUltramarinePkgdbCollections *self,
                           PkgdbItem                      *item)
{
	GsApp *app;
	...
	app_id = g_strdup_printf ("org.ultramarine-linux.ultramarine-%u", item->version);
	...
	gs_app_set_summary (app, GS_APP_QUALITY_LOWEST,
			    /* TRANSLATORS: this is a title for Ultramarine distro upgrades */
	...
	/* show a Ultramarine magazine article for the release */
	url = g_strdup_printf ("https://ultramarine-linux.org/release-announcements/%u",
			       item->version);
...
static gboolean
_is_valid_upgrade (GsPluginUltramarinePkgdbCollections *self,
                   PkgdbItem                      *item)
{
...
static GPtrArray *
load_json (GsPluginUltramarinePkgdbCollections  *self,
           GError                         **error)
{
...
static void
_ensure_cache_async (GsPluginUltramarinePkgdbCollections *self,
                     GCancellable                   *cancellable,
                     GAsyncReadyCallback             callback,
                     gpointer                        user_data)
{
...
ensure_refresh_cb (GObject      *source_object,
                   ...
                   gpointer      user_data)
{
	GsPluginUltramarinePkgdbCollections *self = GS_PLUGIN_ULTRAMARINE_PKGDB_COLLECTIONS (source_object);
	g_autoptr(GTask) task = g_steal_pointer (&user_data);
...
static GPtrArray *
_ensure_cache_finish (GsPluginUltramarinePkgdbCollections  *self,
                      GAsyncResult                    *result,
                      GError                         **error)
{
...
static void
gs_plugin_ultramarine_pkgdb_collections_list_distro_upgrades_async (GsPlugin                        *plugin,
                                                               ...
                                                               gpointer                         user_data)
{
	...
	task = g_task_new (plugin, cancellable, callback, user_data);
	g_task_set_source_tag (task, gs_plugin_ultramarine_pkgdb_collections_list_distro_upgrades_async);
...
static GsAppList *
gs_plugin_ultramarine_pkgdb_collections_list_distro_upgrades_finish (GsPlugin      *plugin,
                                                                GAsyncResult  *result,
                                                                GError       **error)
{
	return g_task_propagate_pointer (G_TASK (result), error);
...
static gboolean
refine_app (GsPluginUltramarinePkgdbCollections *self,
            GPtrArray                      *distros,
            GsApp                          *app,
...
static gboolean
gs_plugin_ultramarine_pkgdb_collections_refine_finish (GsPlugin      *plugin,
                                                  GAsyncResult  *result,
                                                  GError       **error)
{
	return g_task_propagate_boolean (G_TASK (result), error);
...
static void
gs_plugin_ultramarine_pkgdb_collections_class_init (GsPluginUltramarinePkgdbCollectionsClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);
	GsPluginClass *plugin_class = GS_PLUGIN_CLASS (klass);

	object_class->dispose = gs_plugin_ultramarine_pkgdb_collections_dispose;
	object_class->finalize = gs_plugin_ultramarine_pkgdb_collections_finalize;

	plugin_class->setup_async = gs_plugin_ultramarine_pkgdb_collections_setup_async;
	plugin_class->setup_finish = gs_plugin_ultramarine_pkgdb_collections_setup_finish;
	plugin_class->refine_async = gs_plugin_ultramarine_pkgdb_collections_refine_async;
	plugin_class->refine_finish = gs_plugin_ultramarine_pkgdb_collections_refine_finish;
	plugin_class->refresh_metadata_async = gs_plugin_ultramarine_pkgdb_collections_refresh_metadata_async;
	plugin_class->refresh_metadata_finish = gs_plugin_ultramarine_pkgdb_collections_refresh_metadata_finish;
	plugin_class->list_distro_upgrades_async = gs_plugin_ultramarine_pkgdb_collections_list_distro_upgrades_async;
	plugin_class->list_distro_upgrades_finish = gs_plugin_ultramarine_pkgdb_collections_list_distro_upgrades_finish;
}

GType
gs_plugin_query_type (void)
{
	return GS_TYPE_PLUGIN_ULTRAMARINE_PKGDB_COLLECTIONS;
}
